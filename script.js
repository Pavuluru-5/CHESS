    ...
    document.querySelectorAll('.box').forEach(image => {
        if (image.innerText.length !== 0) {
            if (image.innerText == 'Wpawn' || image.innerText == 'Bpawn') {
                image.innerHTML = `${image.innerText} <img class='all-img all-pown' src="${image.innerText}.png" alt="" height = 40 width =3 >`
                image.style.cursor = 'pointer'
            }
            else {
                image.innerHTML = `${image.innerText} <img class='all-img' src="${image.innerText}.png" alt="" height = 40 width =3>`
                image.style.cursor = 'pointer'
            }
        }
    })
...
function checkGameOver() {
    ...
    // Loop through all boxes to check for kings
    document.querySelectorAll('.box').forEach(box => {
        if (box.innerText === 'Wking') {
            whiteKingPresent = true;
        } else if (box.innerText === 'Bking') {
            blackKingPresent = true;
        }
    });

    // If one of the kings is missing, game over
    if (!whiteKingPresent || !blackKingPresent) {
        // document.getElementById('game-over').innerText = "Game Over!";
        window.location.href = 'game_over.html';
    }
    else{
        document.getElementById('game-over').innerText = "Its not Over yet!";
    }
}
